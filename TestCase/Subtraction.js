...
describe('Calculator Subtract operation', function () {
    beforeEach(function () {
        console.log("Navigate to URL and Maximize Browser Window");
        obj.getURL();
        browser.manage().window().maximize();
    });
    using(dd.DataDrivenSub, function (data, description) {
        it('Perform Subtraction and verify Result-'+description, function () {
            obj.firstEditBox.sendKeys(data.firstinput);
            obj.lastEditBox.sendKeys(data.secondinput);
            obj.subtraction.click();
